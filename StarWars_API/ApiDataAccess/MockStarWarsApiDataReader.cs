...
{
    //Use this class in case SWAPI is down
    public class MockStarWarsApiDataReader : IApiDataReader
    {
        private const string ExpecredBaseAddress = "https://swapi.dev/";
        private const string ExpectedRequestUri = "api/planets";


        public Task<string> Read(string baseAddress, string requestUri)
        {
            if (baseAddress != ExpecredBaseAddress)
            {
                throw new ArgumentException($"Base address can only be {ExpecredBaseAddress}");
            }
